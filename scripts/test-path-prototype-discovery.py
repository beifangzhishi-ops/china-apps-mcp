...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python scripts/test-path-prototype-discovery.py https://host.example/cam/mcp")
        return 2
    ...
    with httpx.Client(timeout=15.0, follow_redirects=False) as client:
        ...
        for url in resource_candidates:
            ...
            print(f"Resource metadata candidate {url}: HTTP {response.status_code}")
        ...
        if len(authorization_servers) != 1:
            raise SystemExit("Expected exactly one authorization server for the isolated CAM prototype.")
        ...
        for url in _authorization_metadata_candidates(issuer):
            response = client.get(url)
            print(f"Authorization metadata candidate {url}: HTTP {response.status_code}")
            if response.is_success:
        ...
        for field in ("authorization_endpoint", "token_endpoint", "registration_endpoint", "revocation_endpoint"):
            ...
            if value and not value.startswith(expected_prefix):
                raise SystemExit(f"{field} escaped the CAM issuer path prefix: {value}")

        print(f"\nProtected-resource metadata used: {resource_metadata_url}")
        print(f"Authorization metadata used:      {metadata_url}")
        print("CAM path-prototype OAuth discovery looks ready for ChatGPT.")
    return 0
